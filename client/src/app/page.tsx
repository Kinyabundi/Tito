			...
			<div className="mt-8 grid grid-cols-1 md:grid-cols-8 pl-20 gap-8">
				<div className="col-auto md:col-span-4 mt-12 space-y-10">
					<div className="text-6xl font-bold leading-tight">
						...
						<span className="text-transparent bg-gradient-to-r bg-clip-text from-[#6366F1] via-[#8B5CF6] to-[#EC4899]">
							X402 Payments
						</span>
					</div>
					<div className="mt-8">
						<p className="text-xl text-gray-300 leading-relaxed">
							Join Netflix, Spotify, and thousands of service providers using Tito's AI-powered platform. 
							Create subscription services with instant X402 payments and automated billing management.
						</p>
					</div>
				...
				<div className="w-full relative z-20">
					<div className="text-center space-y-4">
						...
						<p className="text-xl text-gray-300 px-10 max-w-4xl mx-auto">
							Join the future of subscription services with X402 blockchain payments, AI-powered customer management, 
							and real-time analytics that help you grow your business.
						</p>
					</div>
					<div className="grid grid-cols-1 md:grid-cols-4 gap-6 mt-16">
						<FeatureCard 
							title="Instant X402 Payments" 
							description="Receive payments instantly with blockchain security. No waiting periods, no chargebacks." 
							icon={<TbCreditCard className="w-8 h-8" />} 
						/>
						<FeatureCard 
							title="AI Customer Management" 
							description="Tito AI handles customer inquiries, renewals, and cancellations automatically 24/7." 
							icon={<TbRobot className="w-8 h-8" />} 
						/>
						<FeatureCard 
							title="Real-time Analytics" 
							description="Track subscriptions, revenue, churn rates, and customer insights in real-time." 
							icon={<TbChartLine className="w-8 h-8" />} 
						/>
						<FeatureCard 
							title="Easy Integration" 
							description="Simple webhook setup and comprehensive documentation for quick integration." 
							icon={<TbBox className="w-8 h-8" />} 
						/>
					</div>
				</div>
			</div>
					...
					<div className="pt-5">
						...
						<div className="space-y-6">
							<StepItem 
								...
								description="Create your provider account with business details, wallet address, and verification documents." 
							/>
							<StepItem 
								title="Design Your Services" 
								description="Set up subscription tiers, pricing models, trial periods, and service features through our dashboard." 
							/>
							<StepItem 
								title="Configure Integration" 
								description="Add payment endpoints, configure webhooks, and test the integration with our sandbox environment." 
							/>
							<StepItem 
								title="Launch & Monitor" 
								description="Go live with your services and monitor subscriptions, revenue, and customer analytics in real-time." 
							/>
						</div>
					</div>
				</div>
...
const HomeLink = ({ text, isActive, href }: HomeLinkProps) => {
	return (
		<Link href={href ?? "/"}>
			<p className={cn(
				"transition-all duration-300 hover:text-[#6366F1]",
				isActive 
					? "text-[#6366F1] font-semibold" 
					: "text-gray-300 hover:font-semibold"
			)}>
				{text}
			</p>
		</Link>
			...
			<div className="p-[1px] rounded-2xl bg-gradient-to-r from-[#6366F1] to-[#8B5CF6]">
				<div className="px-6 py-8 bg-[#1F2937] rounded-2xl h-full">
					<div className="flex items-center justify-between mb-4">
						<div className="p-3 bg-gradient-to-br from-[#6366F1]/20 to-[#8B5CF6]/20 rounded-xl text-[#6366F1]">
							{icon}
						</div>
